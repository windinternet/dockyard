...
async function startTarget(target: string): Promise<unknown> { const [projectResult, applicationResult] = await Promise.all([daemonGet('/api/projects') as Promise<{ projects: Array<{ id: string; name: string }> }>, daemonGet('/api/applications') as Promise<{ applications: Array<{ id: string; projectId: string; name: string }> }>]); const project = projectResult.projects.find((item) => item.name === target); if (project) return daemonPost(`/api/projects/${encodeURIComponent(project.id)}/start`); const projectsById = new Map(projectResult.projects.map((item) => [item.id, item])); const matches = applicationResult.applications.filter((item) => { const owner = projectsById.get(item.projectId); if (!owner) return false; const qualified = `${owner.name}:${item.name}`; const compact = item.name.startsWith(`${owner.name}:`) ? item.name : null; return target === qualified || target === compact; }); if (matches.length > 1) fail(`应用标识不唯一：${target}。请使用 project:完整应用名。`, 4); const application = matches[0]; if (!application) fail(`找不到项目或应用：${target}`, 4); return daemonPost(`/api/applications/${encodeURIComponent(application.id)}/start`); }
