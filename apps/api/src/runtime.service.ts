...
export class RuntimeService implements OnModuleDestroy {
  ...
  async startProject(projectId: string): Promise<Application[]> { this.project(projectId); const applications = this.database.db.listApplications(projectId); return Promise.all(applications.map((application) => this.start(application.id))); }
