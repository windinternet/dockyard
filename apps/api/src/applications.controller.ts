...
function runtimeEvent(update: RuntimeUpdate): MessageEvent { return update.type === 'application' ? { event: 'application', data: publicApplication(update.application) } as unknown as MessageEvent : update.type === 'metric' ? { event: 'metric', data: update.metric } as unknown as MessageEvent : { event: 'project', data: update.project } as unknown as MessageEvent; }
