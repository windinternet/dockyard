...
4. 外部运行时消失且不是用户明确停止时，沿用 `never` / `on-failure` / `always`、重试预算、退避和稳定窗口；由于外部进程没有可读取的退出码，「进程消失」明确作为异常，触发 `on-failure`；恢复操作由守护进程串行执行。
5. 守护进程关闭不会终止外部启动的进程。显式停止/重启在 Web 确认后，仍会重新核验 PID、启动时间与工作目录，再向已匹配的外部 PID 发送 `SIGTERM`。
...
- 同一 Application 命中进程树中的多个进程时，优先选择有监听端口的进程作为 PID，并聚合该组端口；由 Dockyard 启动的应用也从同一组进程读取端口。
